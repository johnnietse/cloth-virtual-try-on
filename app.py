...
def get_shirt_list():
    """Fetch the list of shirt images dynamically from the directory."""
    # Use a temporary directory or store shirts in a non-persistent way if required.
    return os.listdir(
        'Resources/Shirts')  # You can dynamically fetch the shirt images from an external location or database.
...
@app.route('/upload_shirt', methods=['POST'])
def upload_shirt():
    """Handle shirt image uploads, save them temporarily."""
    if 'shirt_image' not in request.files:
        return jsonify({"error": "No file part"}), 400
    file = request.files['shirt_image']
    if file.filename == '':
    ...
    if file:
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            ...
            temp_shirt_path = temp_file.name
        return jsonify({"message": "Shirt image uploaded successfully!"}), 200


@app.route('/upload', methods=['POST'])
def upload_video():
    """Handle video upload and process it using temporary files."""
    if 'file' not in request.files:
        return jsonify({"error": "No file part"}), 400
    file = request.files['file']
    if file.filename == '':
    ...
    if file:
        ...
        # Use tempfile to store the uploaded video temporarily
        ...
        # Process video and save the result in a temporary file
        processed_video_path = process_video(video_path, video_filename, shirt_index)

        # Return JSON with the processed video URL
        return jsonify({
            "message": "Video processing complete! Click the link below to download.",
            "download_url": processed_video_path
        })


def process_video(input_path, filename, shirt_index):
    """Process video and overlay shirt on the person's body."""
    detector = PoseDetector()
    cap = cv2.VideoCapture(input_path)

    # Temporary file for processed video output
    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as temp_file:
        ...
        while True:
            ...
            if lmList and len(lmList) > 24:
                ...
                imgShirt = cv2.imread(os.path.join('Resources/Shirts', listShirts[shirt_index]), cv2.IMREAD_UNCHANGED)
                height, width = imgShirt.shape[:2]
...
def overlay_transparent(background, overlay, alpha_blend=0.7):
    """Overlay a transparent shirt on a video frame."""
    b, g, r, a = cv2.split(overlay)
