...
def create_temp_dir():
    ...
    os.makedirs(os.path.join(temp_dir, 'shirts'), exist_ok=True)
    return temp_dir
...
SHIRT_FOLDER = os.path.join(temp_dir, 'shirts')

app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['PROCESSED_FOLDER'] = PROCESSED_FOLDER
app.config['SHIRT_FOLDER'] = SHIRT_FOLDER

# Dummy function to simulate shirt list (replace with actual shirt files loading if needed)
def get_shirt_list():
    """Fetch the list of shirt images dynamically from the directory."""
    return os.listdir(SHIRT_FOLDER) if os.path.exists(SHIRT_FOLDER) else []
...
def upload_shirt():
    """
    Upload a shirt image to the temporary 'Shirts' directory.
    """
    if 'shirt_image' not in request.files:
        return jsonify({"error": "No file part"}), 400
    file = request.files['shirt_image']
    if file.filename == '':
        return jsonify({"error": "No selected file"}), 400
    if file:
        # Save the uploaded shirt to the temporary 'Shirts' directory
        filepath = os.path.join(SHIRT_FOLDER, file.filename)
        file.save(filepath)
        return redirect(url_for('index'))  # Redirect back to the home page to display the updated list
...
def upload_video():
    ...
    if file:
        ...
        # Get selected shirt index (default to 0 if not provided)
        shirt_index = int(request.form.get('shirt_index', 0))
        processed_filepath = process_video(filepath, filename, shirt_index)
        processed_url = f"/{processed_filepath}"
...
def process_video(input_path, filename, shirt_index):
    ...
    while True:
        ...
        if lmList and len(lmList) > 24:
            ...
            # Load the shirt image
            imgShirt = cv2.imread(os.path.join(SHIRT_FOLDER, listShirts[shirt_index]), cv2.IMREAD_UNCHANGED)
...
@app.route('/<path:filepath>')
def download_file(filepath):
    return send_from_directory(temp_dir, filepath)
